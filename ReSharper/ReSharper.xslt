...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:import href="http://jetbrains.com/resharper/updates/online.Updates.xslt" />

  <xsl:template match="/">
    <xsl:apply-templates>
      <xsl:with-param name="ReleaseMajor" select="$ReleaseMajor" />
      <xsl:with-param name="ReleaseMinor" select="$ReleaseMinor - 1" />
      <xsl:with-param name="ReleaseBuild" select="0" />
